...
```{r}
aus_molluscs <- read_xlsx("data/WoRMS_Mollusca_australia_20230621.xlsx")
...
output_df <- readRDS("outputs/worrms_Australia_wm_records_taxamatch")
...
output_df |> 
  filter(duplicated(search_term)) |> 
  select(search_term, ends_with("name"), authority, match_type, valid_authority)  |> 
  count(match_type)
```
...
setdiff(new, unique(molluscs_afd_authority$scientific_name)) |> length()
