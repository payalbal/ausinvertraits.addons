---
  title: "AFD checklist"
author: "Payal Bal, Fonti Kar, Hannah Smart"
date: "2022-08-30"
...
## Functions 
source(file.path(getwd(),"scripts/remove_improper_names_v2.R")) # Version Fonti created
```

### Australian Faunal Directory taxonomy

```{r}
afd_species <- read_csv(file.path(getwd(), "data/afd_Jan2023_mergedCols.csv"))
...
distribution <- fread(file.path(getwd(), "data/dwca-griis-australia-v1.6/distribution.txt"))
species <- fread(file.path(getwd(), "data/dwca-griis-australia-v1.6/speciesprofile.txt"))
taxa <- fread(file.path(getwd(), "data/dwca-griis-australia-v1.6/taxon-edited.txt"), fill = TRUE)
...
sum(is.na(griis_list$scientificName))
...
```{r}
griis_species <- read.csv(file.path(getwd(), "data/GRIIS_Global_Register_of_Introduced_and_Invasive_Species_Australia.csv"))
...
message("Removing AFD species with GRIIS ...")
...
species <- fread(file.path(getwd(),"data/worms/WoRMS_download_2022-10-01", "speciesprofile.txt"), quote = "")
taxa <- fread(file.path(getwd(), "data/worms/WoRMS_download_2022-10-01", "taxon.txt"), quote = "")
