...
d1_mod <- d1 %>%
  ...
  dplyr::mutate(functional_role_adult = case_when(
    ...
    Feeding_Group == "pollinator" ~ "herbivore pollinator",
    functional_role_adult == "scavenger" ~ NA,
  ...
  dplyr::mutate(functional_role_larva = case_when(
    ...
    food == "caterpillar feeder" ~ "predator",
    food == "dead animals" ~ "necrophage",
    food == "dead plant material" ~ "detritivore",
    food == "dead plant material/roots" ~ "detritivore herbivore",
    food == "dead wood" ~ "xylophage",
    ...
    food == "plant material/insect larvae" ~ "omnivore",
    food == "sap pollen nectar, larvae eat wood" ~ "xylophage",
    food == "Scaritini" ~ "predator",
  ...
  dplyr::mutate(usual_habitat_larvae = case_when(
    ...
    usual_habitat_larvae == "subterranean" ~ "soil",
    food == "dead wood" ~ "in_dead_wood_ground in_standing_wood_dead",
...
d1_template <- template %>%
  ...
  dplyr::mutate(value_type = case_when(trait_name == "body_length" ~ "median", .default = as.character(value_type))) %>%  # add value_type for the one numerical trait
